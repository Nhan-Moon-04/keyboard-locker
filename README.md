...
Use the tray menu item "Open settings.ini" to edit the configuration file. The script now prefers Notepad++ if it is installed, and falls back to Notepad otherwise.

## Building the executable

You do not need to rebuild anything after editing `settings.ini`; just restart the app.

If you change the script itself and want a new executable, compile it with:

```powershell
& "C:\Program Files\AutoHotkey\Compiler\Ahk2Exe.exe" /in "D:\DO\ahk-keyboard-locker-1.0\keyboard-locker.ahk" /out "D:\DO\ahk-keyboard-locker-1.0\keyboard-locker_patched.exe" /icon "D:\DO\ahk-keyboard-locker-1.0\unlocked.ico" /bin "C:\Program Files\AutoHotkey\Compiler\Unicode 64-bit.bin"
```
